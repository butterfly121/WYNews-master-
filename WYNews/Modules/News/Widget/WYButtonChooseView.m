...
{
    WYLabelButton*firstBtn=self.buttonArray[0];
    return firstBtn;
}
    ...
    [UIView animateWithDuration:kDuration animations:^{
        ...
        for (int i  = 0; i < self.buttonArray.count; i++) {
            ...
            if (i == 0) {
                buttonX = kMarginW;
                buttonY = kMarginH;
                if ([button.titleLabel.text isEqualToString:@"头条"]&&self.edit==NO)
                {
                    button.selected=YES;
...
{
    if ([self.chooseDelegate respondsToSelector:@selector(didSelectedButton:selextedBtn:)]) {
        WYLabelButton*selectedBtn=self.buttonArray[0];
        [self.chooseDelegate didSelectedButton:button selextedBtn:selectedBtn];
    }
}
